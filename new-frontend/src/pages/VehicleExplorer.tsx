...
Provide ONLY the values in a clear format. If a specification is not found, write "Not specified".`;
  ...
  const fetchFallbackSpecs = async () => {
  const fallbackQuery = `
Give the general specifications for the vehicle ${brand} ${model}.
Provide:
- Top speed
...
If exact model info is unavailable, give typical numbers for cars in this class.
Return ONLY plain values in bullet points.
`;
  ...
  try {
    ...
    const data = await response.json();
    const specs = parseSpecsFromText(data.answer.answer_text);
    return specs;

  } catch (error) {
    console.error("Fallback spec error:", error);
